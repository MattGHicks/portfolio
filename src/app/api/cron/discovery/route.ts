/**
 * Discovery cron — daily 9am ET.
 * Scrapes Ashby + Greenhouse boards for watchlist companies.
 * Inserts new roles with status=unscored. Dedupes by (ats_platform, ats_external_id).
 */
...
type ScrapeResult = { company: string; found: number; new: number; errors: string[] };
...
async function existingActiveRole(company: string, title: string): Promise<boolean> {
  const rows = await db
    ...
    .where(
      and(
        eq(roles.company, company),
        eq(roles.title, title),
        sql`status NOT IN ('archived', 'dropped')`
      )
    )
    .limit(1);
...
async function scrapeAshby(name: string, slug: string): Promise<ScrapeResult> {
  const result: ScrapeResult = { company: name, found: 0, new: 0, errors: [] };
  try {
    ...
    for (const job of board.jobs) {
      ...
      if (inserted.length > 0) result.new++;
    }
  } catch (err) {
